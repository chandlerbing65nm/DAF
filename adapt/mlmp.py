...
class MLMP:
    ...
    def __init__(self, ovss_type, ovss_backbone, lr, classes, vision_outputs=(-1,), 
                 ...
                 device='cpu', token_merge=False, merge_type='algm',
    ...
    def perform_adaptation(self, x, diag_labels=None, diag_ignore_index=255):
        ...
        for iter in range(self.steps):
            ...
            if self.prompt_integration == 'loss':
                logits, image_features, _, cls_logits = self.model(x, self.text_x[:-1], True, interpolate=False,
                ...
                if needs_source:
                    with torch.no_grad():
                        logits_src, image_features_src, _, _ = self.source_model(x, self.text_x[:-1], True, interpolate=False,
                                                      vision_outputs=self.vision_outputs, return_vanilla_cls=True, 
                                                      vision_out_type="mean")

            elif self.prompt_integration == 'text':
                logits, image_features, _ = self.model(x, self.text_x[-1], True, interpolate=False,
                ...
                if needs_source:
                    with torch.no_grad():
                        logits_src, image_features_src, _ = self.source_model(x, self.text_x[-1], True, interpolate=False,
            ...
            if self.module_safs and image_features_src is not None:
                ...
                if not keep_mask.any():
                    ...
                    if cls_logits is not None:
                        cls_logits = cls_logits[:, keep_mask]
            ...
            if self.loss_ent:
                ...
                if cls_logits is not None:
                    ...
                    loss = loss + self.lamb_ent * entropy_per_pixel.mean() + self.alpha_cls * entropy_per_cls.mean()
                else:
            ...
            if self.loss_cmac:
                text_feats = self.text_x[:-1] if self.prompt_integration == 'loss' else self.text_x[-1]
                cmac_loss = self.cmac_loss(image_features, image_features_src, text_feats)
                loss = loss + cmac_loss
