...
@TRANSFORMS.register_module()
class LoadCarlaAnnotations(BaseTransform):
    """Load Carla segmentation annotations.

    Carla stores label IDs in the **red channel** of RGBA PNGs.  Raw IDs are
    remapped to a contiguous 0-13 training-ID space via ``id_to_trainid``.
    """

    id_to_trainid = {
        7: 0, 8: 1, 1: 2, 11: 3, 2: 4, 5: 5, 18: 6, 12: 7,
    ...
    def transform(self, results: dict) -> dict:
        img_bytes = fileio.get(
            ...
            backend_args=results.get('backend_args', None),
        )
        gt_seg_map = mmcv.imfrombytes(
            img_bytes, flag='color', channel_order='rgb',
        ).astype(np.uint8)

        # Carla labels live in the red channel
        red = gt_seg_map[:, :, 0]
        decoded = np.full(red.shape, self.ignore_index, dtype=np.uint8)
        for raw_id, train_id in self.id_to_trainid.items():
            decoded[red == raw_id] = train_id

        results['gt_seg_map'] = decoded
        results['seg_fields'] = ['gt_seg_map']
        return results
